...
from .models import AgentPrompt, ModelTurn, ToolCall
...
class DeterministicDemoModel:
    ...
    def generate(self, prompt: AgentPrompt) -> ModelTurn:
        ...
        if self._is_guess_followup(user_text) and self._history_has_denial(prompt):
            return ModelTurn(text=self._boundary_response(prompt))
    ...
    def _after_tool(self, prompt: AgentPrompt, content: Mapping[str, Any]) -> ModelTurn:
        ...
        return ModelTurn(text=self._no_evidence_response(prompt))
    ...
    def _grounded_response(self, prompt: AgentPrompt, fact: Mapping[str, Any]) -> ModelTurn:
        ...
        if "先报结论" in prompt.character.speech_style:
            text = f"已确认：{statement}"
        elif "少废话" in prompt.character.surface_traits:
            text = statement
        else:
            text = f"公开能确认的是：{statement}"
        return ModelTurn(text=text, source_lore_ids=(lore_id,))

    def _boundary_response(self, prompt: AgentPrompt) -> str:
        ...
        if runtime.active_incident_ids:
            ...
            prefix = "我没有这部分可核实的资料。"
        if "先报结论" in prompt.character.speech_style:
            return f"结论：目前不能确认。{prefix}缺少可核实来源，我不能替内部记录下判断。"
        if "少废话" in prompt.character.surface_traits:
            return f"{prefix}完整内部结论没有可核实来源，我不会猜。"
        return f"{prefix}内部最后怎么落笔，我没看到，不能替那份记录补台词。"
