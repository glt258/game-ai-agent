...
## 角色技能设计 CS-S0.1（contract v0.1.1）
...
**Canonical taxonomy boundary（canonical taxonomy 边界）**：CI-B1.5 对跨 taxonomy 输入采用 fail closed。非 canonical 角色值进入 `combat_role_profile` 属边界违规；legacy flat alias seam 不适用于该 canonical profile，禁止自动 normalization，非法值不得写入 request 的 canonical profile。
